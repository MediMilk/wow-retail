...
function ilc:createDropdown(opts)
	...
	dd_title:SetFontObject(ilc:get_font(14))
...
function ilc:createToggle(opts)
	...
	text:SetText(title)
	text:SetFontObject(ilc:get_font(14))
	text:ClearAllPoints()
...
function ilc:createColor(opts)
	...
	picker:SetBackdrop({bgFile = ilc.media.flat, edgeFile = ilc.media.flat, edgeSize = 2, insets = {top = 2, right = 2, bottom = 2, left = 2}})
	...
	picker.text = picker:CreateFontString(nil, "OVERLAY", "ilc_font")
...
function ilc:createEdit(opts)
	...
	input:SetFontObject(ilc:get_font(14))
	...
	ilc:setBackdrop(input)
	input:SetBackdropColor(.18,.22,.25,1)

	local label = input:CreateFontString(nil, "OVERLAY", "ilc_font")
	label:SetText(title)
...
function ilc:createList(opts)
	...
	ilc:setBackdrop(container)
	container:SetBackdropColor(.18,.22,.25,1)

	--scrollframe 
	local content = ilc:createScrollFrame(container)
	...
	container.content.text:SetFontObject(ilc:get_font(13))
	...
	ilc:setBackdrop(container.insert)
	container.insert:SetBackdropColor(.10,.14,.17,1)
	container.insert:SetFontObject(ilc:get_font(13))
	...
	container.button:SetBackdrop({
		bgFile = ilc.media.flat, 
		edgeFile = ilc.media.flat, edgeSize = 1,
		insets = { left = 1, right = 1, top = 1, bottom = 1 }
	})
	container.button:SetBackdropColor(unpack(ilc.media.blue))
	container.button:SetBackdropBorderColor(unpack(ilc.media.border))
	...
	container.button.x:SetFont(ilc.media.font, 12)
