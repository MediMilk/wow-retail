...
-- Debug
function ilc:print(...)
	print("|cffA02C2FILC:|r", ...)
end

function ilc:debug(...)
	if (ilc.config.debug or ilc.enabledebug) then
		ilc:print(...)
	end
end

-- Pixel Perfect
function ilc:calculate_scale()
	ilc.scale = 768 / select(2, GetPhysicalScreenSize())
	ilc.pixel = ilc.scale / GetCVar("uiScale") or 1
	ilc.border = ilc.pixel * 2
end
ilc:calculate_scale()

function ilc:get_border(frame)
...
-- UID hash string
function ilc:GetItemID(itemLink)
...
function ilc:GetItemUID(itemLink, lootedBy, rollID)
	lootedBy = lootedBy and ilc:FetchUnitName(lootedBy) or ""
	local itemString = string.match(itemLink, "item[%-?%d:]+")

	if (not itemString) then 
		ilc:debug(itemLink.." isn't actually an item. Please report this to the developer")
...
-- Messages
--=====================================
function ilc:sendAction(action, ...)
	local delim = ilc.deliminator
	local paramString = strjoin(delim, ...)

	-- allow the user to whisper through this function
	local recipient = ilc.overrideRecipient or ilc:FetchUnitName('player')
	local priority = ilc.overridePriority or "NORMAL"
	local channel = "WHISPER"
	if (IsInRaid() or IsInGroup() or UnitInRaid("player")) then 
		channel = "RAID"
	end
	channel = ilc.overrideChannel or channel

	-- merge then send
	local data = action..delim..paramString
	-- ilc:debug(ilc.message_prefix, data, channel, recipient, priority)
	ilc.comm:SendCommMessage(ilc.messagePrefix, data, channel, recipient, priority)

	-- unset these, probably shouldn't have them in the first place but it works
	ilc.overrideChannel = nil
	ilc.overrideRecipient = nil
	ilc.overridePriority = nil
end
...
function ilc:RGBPercToHex(r, g, b)
...
function ilc:setBackdrop(frame, r, g, b, a)
	local backdrop = r and {r, g, b, a} or ilc.media.backdrop
	local r, g, b, a = unpack(backdrop)
	a = a or 1

	local border = ilc:get_border(frame)

	frame:SetBackdrop({bgFile = ilc.media.flat, edgeFile = ilc.media.flat, edgeSize = border})
	frame:SetBackdropColor(r, g, b, a)
	frame:SetBackdropBorderColor(unpack(ilc.media.border))
end

function ilc:skinButton(f, small, color)
	local colors = ilc.media.backdrop
	local hovercolors = ilc.media.hover
	if (color == "red") then
		colors = {.6, .1, .1, 0.6}
		hovercolors = {.6, .1, .1, 1}
	...
	f:SetBackdrop({bgFile = ilc.media.flat, edgeFile = ilc.media.flat, edgeSize = ilc.border})
	f:SetBackdropColor(unpack(colors)) 
    f:SetBackdropBorderColor(0, 0, 0, 1)
    f:SetNormalFontObject(ilc:get_font(14, "OUTLINE"))
	f:SetHighlightFontObject(ilc:get_font(14, "OUTLINE"))
	f:SetPushedTextOffset(0,-1)
	
	f:SetSize(f:GetTextWidth()+16, 24)

	if (small) then
		f:SetNormalFontObject(ilc:get_font(11, "OUTLINE"))
		f:SetHighlightFontObject(ilc:get_font(11, "OUTLINE"))
	end
...
function ilc:createScrollFrame(parent)
	local frameHolder;
	...
	self.scrollframe = self.scrollframe or CreateFrame("ScrollFrame", "ILC_Scroll", self, "UIPanelScrollFrameTemplate")
...
local function find_compare(itemLink, search)
	...
	ilc.tt:SetHyperlink(itemLink)
	local itemString = string.match(itemLink, "item[%-?%d:]+")
	
	local num_lines = ilc.tt:NumLines(true)
	for i = 1, num_lines do
		local line = _G['ILC:TooltipScanTextLeft'..i] and _G['ILC:TooltipScanTextLeft'..i]:GetText() and _G['ILC:TooltipScanTextLeft'..i]:GetText():utf8lower() or nil;
...
function ilc:fetchUserGear(unit, itemLink)
	local name, link, quality, iLevel, reqLevel, class, subclass, maxStack, equipSlot, texture, vendorPrice = GetItemInfo(itemLink)
	-- local isRelic = ilc:IsRelic(itemLink)
	local isTier, tierType, usable = ilc:isTier(itemLink)
	...
	-- if (isRelic) then
	-- 	local relicType = ilc:GetRelicType(itemLink)
	-- 	local relic1, relic2 = ilc:GetRelics(relicType)
	...
	if (slotID == 0) then
		ilc:print("Can't find compare for "..itemLink..". Let the developer know");
	end
	
	return itemLink1, itemLink2
end

function ilc:GetItemValue(itemLink)
	ilc.tt:SetOwner(UIParent, 'ANCHOR_NONE')
	ilc.tt:SetHyperlink(itemLink)
	local itemString = string.match(itemLink, "item[%-?%d:]+")
	...
	-- Get Wf/TF
	for i = 1, 4 do
		local text = _G['ILC:TooltipScanTextLeft'..i] and _G['ILC:TooltipScanTextLeft'..i]:GetText() and _G['ILC:TooltipScanTextLeft'..i]:GetText():utf8lower() or nil;
		if text then
...
function ilc:itemValidForSession(itemLink, lootedBy, test, rollID)
	lootedBy = lootedBy and ilc:FetchUnitName(lootedBy) or ""
	local valid = false
	...
	if (ilc.loot_sessions[itemUID] == lootedBy and not test) then
		ilc:debug("Session already exists for this item")
		return false
	end
	...
	if (test) then
		ilc:print(itemLink, "is: ")
		ilc:print("Tier: ", isTier and "Yes" or "No")
		if (isTier) then
			ilc:print("Tier Type: ", tierType)
			ilc:print("Tier Usable: ", usable)
		end
		-- ilc:print("Relic: ", isRelic and "Yes" or "No")
		ilc:print("Equipable: ", ((equipSlot or usable) and string.len(equipSlot or usable) > 0) and "Yes" or "No")
		ilc:print("Equip Slot: ", ((equipSlot or usable) and string.len(equipSlot or usable) > 0) and equipSlot or tierType)
		ilc:print("Tradable: ", ilc:verifyTradability(itemLink) and "Yes" or "No")
	end
	...
	if (ilc.forceSession) then
		value = true
		ilc.forceSession = false
	end

	return valid
end

function ilc:itemEquippable(itemUID)
	...
	ilc:print("Experimental: You automatically passed on ", itemLink)
	return false
end

function ilc:isTier(itemLink)
	local isTier = false
	...
	for i = 1, 150 do
		local line = _G['ILC:TooltipScanTextLeft'..i]
...
function ilc:IsRelic(relicLink)
	...
	local isRelic = false
	ilc.tt:SetOwner(UIParent, 'ANCHOR_NONE')
	ilc.tt:SetHyperlink(relicLink)
	
	local subclass = select(7, GetItemInfo(relicLink))
	if (subclass and ilc:RelicString(subclass)) then
		isRelic = true
	end
	
	for i = 2, 6 do
		local text = _G['ILC:TooltipScanTextLeft'..i] and _G['ILC:TooltipScanTextLeft'..i]:GetText() or nil;
		if (text and ilc:RelicString(text)) then
			isRelic = true
			break
		end
	end
...
function ilc:GetRelicType(relicLink)
	local relicType
...
function ilc:GetRelics(rt)
	...
	for relicSlotIndex = 1, C_ArtifactUI.GetNumRelicSlots() do
		...
		if (relicLink) then
			local relicType = ilc:GetRelicType(relicLink)
...
-- Trading
--================================================
function ilc:populate_trade_window(itemLink1)
	local itemLink1 = ilc:GetItemUID(itemLink1, false, -1)
	...
	for bag = 0, 4 do
		for slot = 1, _GetContainerNumSlots(bag) do
			...
			-- local bagUID = bagItemLink and ilc:GetItemUID(bagItemLink, false, -1) or false
			-- local itemUID = ilc:GetItemUID(itemLink, false, -1)
			if (bagItemLink) then
				local itemLink2 = ilc:GetItemUID(bagItemLink, false, -1)
...
-- Tradability
function ilc:tradableTooltip(itemLink)
		...
		for slot = 1, _GetContainerNumSlots(bag) do
			...
			if (bagUID and bagUID == itemUID) then
				...
				for i = 1, 150 do
					...
					if (string.find(text, tradableString) ~= nil or (ilc.enableTests and string.find(text, sellableString) ~= nil)) then
						isTradable = true
						break
					end
				end
...
function ilc:verifyTradability(itemLink)
	if (GetItemInfo(itemLink)) then
		ilc:debug("checking if item is tradable", itemLink)
		if (ilc:tradableTooltip(itemLink)) then
			return true
		end
	else
		local itemID = ilc:getItemID(itemLink)
		ilc.items_waiting_for_verify[itemID] = itemLink
...
function ilc:capitalize(str)
	return str:gsub("^%l", string.utf8upper)
end

-- returns a nice readable format
function ilc:unitName(str)
	local name, server = strsplit("-", str)

	return ilc:capitalize(Ambiguate(name, "short"))
end

-- To colorize lootedBy player
function ilc:prettyName(playerName)
	local name = ilc:unitName(playerName)

	local classFileName = select(2, UnitClass(name)) or select(2, UnitClass(playerName)) or playerClass or ilc.demo_samples.classes[math.random(#ilc.demo_samples.classes)]
	local color = RAID_CLASS_COLORS[classFileName] or {["r"] = 1, ["g"] = 1, ["b"] = 1}

	return "|cff"..ilc:RGBToHex(color)..name.."|r", color
end


-- returns name-server for any valid name or unit
function ilc:FetchUnitName(name_string)
