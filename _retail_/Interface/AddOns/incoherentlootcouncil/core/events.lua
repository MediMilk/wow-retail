...
events:SetScript("OnEvent", function(self, event, arg1, arg2)
	...
	if (event == "LOOT_OPENED" and IsInRaid()) then
		...
		-- 		if (num_free == 0) then
		-- 			ilc:print("You have full bags! Delete something to loot "..itemLink)
		-- 		end

		-- 		-- alert my raid that this exists
		-- 		if (num_free == 0) then
		-- 			SendChatMessage("ILC: I have full bags but I looted "..itemLink, "RAID")
		-- 		elseif (ilc:IsInRaidInstance()) then
		-- 			LootSlot(slot)
		-- 		end
		-- 	end
		-- end
	...
	if (event == "TRADE_SHOW") then
		...
		local itemLink = ilc:fetch_trade_assignment('player', target)
		if (itemLink) then
			ilc:populate_trade_window(itemLink)
		end
		return
	end

	if (event == "TRADE_CLOSED") then
		ilc.is_trading = false
		return
	end
	...
	if (event == "START_LOOT_ROLL" and ilc:IsRaidLeader()) then
		local itemLink = GetLootRollItemLink(arg1)
		ilc:sendAction("startSession", itemLink, ilc:FetchUnitName('player'), 0, arg1)
		return
	end

	-- When a user loots an item, snag that item link and attempt a session
	if (event == "CHAT_MSG_LOOT") then
		ilc:StartSessionFromTradable(nil, arg1, arg2)
