...
function ilc:round(number, decimals)
...
function ilc:get_font(size, outline, shadow)
	size = ilc:round(size)
	...
	if (not ilc.fonts[name]) then
		local font = CreateFont("ILC_"..name)

		font:SetFont(ilc.media.font, tonumber(size), outline)
		if (shadow) then
	...
	return ilc.fonts[name]
end

--======================================
-- Reposition Frames
--======================================
function ilc:repositionFrames()
	-- tabs
	local lasttab = nil
	for tab, v in ilc.tabs:EnumerateActive() do
		-- order entries and loop through them
		local lastentry = false
		for entry, v in ilc:spairs(tab.entries.activeObjects, function(a, b)
			...
			if (entry.itemUID and entry:IsShown()) then
				if (lastentry) then
					...
					entry:SetPoint("TOPLEFT", tab.table.content, "TOPLEFT", ilc.border, 0)
				end
				lastentry = entry
			end
		end
		...
		if (tab.itemUID and tab:IsShown()) then
			if (lasttab) then
				tab:SetPoint("TOPRIGHT", lasttab, "BOTTOMRIGHT", 0, ilc.border)
			else
				tab:SetPoint("TOPRIGHT", ilc.window.tabs, "TOPRIGHT", ilc.border, 0)
			end
			lasttab = tab
		end
	end	
	...
	-- rolls
	local lastroll = nil
	for roll, v in ilc.rolls:EnumerateActive() do
		if (roll.itemUID) then
			if (lastroll) then
				roll:SetPoint("TOPLEFT", lastroll, "BOTTOMLEFT", 0, ilc.border)
			else
				roll:SetPoint("TOPLEFT", ilc.rollFrame, "TOPLEFT", 0, 0)
			end
			lastroll = roll
		end
	end
...
function ilc:getTab(itemUID)
	...
	if (not current_tab) then
		current_tab = ilc.tabs:Acquire()
	end

	return current_tab
end

function ilc:getEntry(itemUID, playerName)
...
--======================================
ilc.assignment_window = CreateFrame("frame", "ILC:Assignments", UIParent, BackdropTemplateMixin and "BackdropTemplate")
ilc.assignment_window:SetSize(370, 228)
ilc.assignment_window:SetPoint("LEFT", 200, 0)
ilc.assignment_window:SetFrameStrata("DIALOG")
ilc.assignment_window:EnableMouse(true)
ilc.assignment_window:SetMovable(true)
-- ilc.assignment_window:SetUserPlaced(true)
ilc.assignment_window:RegisterForDrag("LeftButton")
ilc.assignment_window:SetScript("OnDragStart", function(self) self:StartMoving() end)
ilc.assignment_window:SetScript("OnDragStop", function(self) self:StopMovingOrSizing() end)
ilc.assignment_window:SetScript("OnMouseUp", function(self) self:StopMovingOrSizing() end)
ilc:setBackdrop(ilc.assignment_window)

local title = ilc.assignment_window:CreateFontString(nil, "OVERLAY")
title:SetFontObject(ilc:get_font(12, "OUTLINE"))
title:SetText(ilc.colorString.." - Trade Assignments")
title:SetJustifyH("CENTER")
title:SetPoint("BOTTOM", ilc.assignment_window, "TOP", 0, 2)
ilc.assignment_window.title = title

ilc.assignment_window.lines = {}
for i = 1, 10 do
	local trade_line = CreateFrame("frame", nil, ilc.assignment_window)
	trade_line:SetWidth(ilc.assignment_window:GetWidth())
	...
	trade_line.itemLink.text:SetFontObject(ilc:get_font(14, "OUTLINE"))
	...
	trade_line.lootedBy:SetFontObject(ilc:get_font(14, "OUTLINE"))
	...
	trade_line.arrow:SetFontObject(ilc:get_font(14, "OUTLINE"))
	...
	trade_line.target:SetFontObject(ilc:get_font(14, "OUTLINE"))
	...
	trade_line.remove:SetScript("OnClick", function()
		ilc:remove_trade_assignment(trade_line.data.itemLink, trade_line.data.target)
	end)
	ilc:skinButton(trade_line.remove, true, "red")
	...
	trade_line:Hide()
	ilc.assignment_window.lines[i] = trade_line
end

function ilc.assignment_window:reposition_items()
	local min_height = 100
	local height = 10
	local last
	for i = 1, 10 do
		local line = ilc.assignment_window.lines[i]
		line:ClearAllPoints()

		if (line:IsShown()) then
			if (not last) then
				line:SetPoint("TOP", ilc.assignment_window, 0, -5)
			else
	...
	ilc.assignment_window:SetHeight(height)

	if (not last) then
		ilc.assignment_window:Hide()
	else
		ilc.assignment_window:Show()
	end
end

function ilc:add_trade_assignment(lootedBy, itemLink, target)
	lootedBy = ilc:FetchUnitName(lootedBy)
	target = ilc:FetchUnitName(target)
	local line
	for i = 1, 10 do
		local l = ilc.assignment_window.lines[i]
	...
	line.lootedBy:SetText(ilc:prettyName(lootedBy))
	line.itemLink:SetItem(itemLink)
	line.target:SetText(ilc:prettyName(target))
	line:Show()

	ilc.assignment_window:reposition_items()
end

function ilc:remove_trade_assignment(itemLink, target)
	target = ilc:FetchUnitName(target)
	local line
	for i = 1, 10 do
		local l = ilc.assignment_window.lines[i]
...
function ilc:fetch_trade_assignment(lootedBy, target)
	lootedBy = ilc:FetchUnitName(lootedBy)
	target = ilc:FetchUnitName(target)
	local line
	for i = 1, 10 do
...
local function create_tab(self)
	...
	local tab = CreateFrame('button', nil, ilc.window.tabs, BackdropTemplateMixin and "BackdropTemplate")
	...
	tab:SetAlpha(0.3)
	tab:EnableMouse(1)
	ilc:setBackdrop(tab, .1,.1,.1,.8);
	tab:SetScript("OnClick", function(self, b)
		if (b == "LeftButton") then
			for other_tab, v in ilc.tabs:EnumerateActive() do
	...
	tab.icon:SetDesaturated(true)
	tab.icon:SetPoint("TOPLEFT", tab, "TOPLEFT", ilc.border, -ilc.border)
	tab.icon:SetPoint("BOTTOMRIGHT", tab, "BOTTOMRIGHT", -ilc.border, ilc.border)
	
	tab.wfsock = tab:CreateFontString(nil, "ARTWORK")
	tab.wfsock:SetFontObject(ilc:get_font(15, "OUTLINE"))
	tab.wfsock:SetText("")
	...
	vote_table:SetPoint("TOPLEFT", ilc.window, "TOPLEFT", 10, -100)
	vote_table:SetPoint("BOTTOMRIGHT", ilc.window, "BOTTOMRIGHT", -30, 30)
	vote_table:Hide()
	ilc:setBackdrop(vote_table, .1, .1, .1, .8);
	tab.table = vote_table

	local content = ilc:createScrollFrame(vote_table)
	...
	ilc:setBackdrop(content, .1, .2, .1, .8);
	
	-- Headers
	vote_table.name_text = vote_table:CreateFontString(nil, "OVERLAY")
	vote_table.name_text:SetFontObject(ilc:get_font(14))
	...
	vote_table.rank_text:SetFontObject(ilc:get_font(14))
	...
	vote_table.ilvl_text:SetFontObject(ilc:get_font(14))
	...
	vote_table.interest:SetFontObject(ilc:get_font(14))
	...
	vote_table.notes_text:SetFontObject(ilc:get_font(14))
	...
	vote_table.current_text:SetFontObject(ilc:get_font(14))
	...
	vote_table.votes_text:SetFontObject(ilc:get_font(14))
	...
	vote_table.actions_text:SetFontObject(ilc:get_font(14))
	...
	vote_table.item.itemtext:SetFontObject(ilc:get_font(15, "THINOUTLINE"))
	...
	vote_table.item.num_items:SetFontObject(ilc:get_font(14, "THINOUTLINE"))
	...
	vote_table.item.itemdetail:SetFontObject(ilc:get_font(14, "THINOUTLINE"))
	...
	ilc:setBackdrop(vote_table.item.icon, 0,0,0,.8);
	
	vote_table.item.wfsock = vote_table.item.icon:CreateFontString(nil, "ARTWORK")
	vote_table.item.wfsock:SetFontObject(ilc:get_font(15, "OUTLINE"))
	...
	vote_table.item.icon.tex:SetPoint("TOPLEFT", vote_table.item.icon, "TOPLEFT", ilc.border, -ilc.border)
	vote_table.item.icon.tex:SetPoint("BOTTOMRIGHT", vote_table.item.icon, "BOTTOMRIGHT", -ilc.border, ilc.border)
	
	-- num votes left
	vote_table.numvotes = vote_table:CreateFontString(nil, "OVERLAY")
	vote_table.numvotes:SetFontObject(ilc:get_font(12))
	...
	ilc:skinButton(vote_table.endSession,false,"red")
	vote_table.endSession:SetScript("OnClick", function()
		ilc:sendAction("endSession", tab.itemUID);
		
		local itemLink = ilc.itemMap[tab.itemUID]
		ilc:print("Ending session:", itemLink)

		ilc:endSession(tab.itemUID)
	end)
	...
	ilc:setBackdrop(vote_table.info_pane, .1, .1, .1, 1)
	...
	vote_table.info_pane.name:SetFontObject(ilc:get_font(14))
	...
	ilc:skinButton(vote_table.info_pane.award, false, "blue")
	vote_table.info_pane.award:SetScript("OnClick", function(self)
		ilc:awardLoot(vote_table.info_pane.playerName, vote_table.info_pane.itemUID)
		vote_table.info_pane:Hide()
	end)
	...
	-- history:SetPoint("TOPLEFT", vote_table.info_pane, "BOTTOMLEFT", 0, ilc.border)
	-- history:SetPoint("TOPRIGHT", vote_table.info_pane, "BOTTOMRIGHT", 0, ilc.border)
	-- ilc:setBackdrop(history)
	-- history:SetHeight(100)

	-- history.text = history:CreateFontString(nil, "OVERLAY")
	-- history.text:SetFontObject(ilc:get_font(14))
	...
	-- 	line.item.tex:SetPoint("TOPLEFT", line.item, "TOPLEFT", ilc.border, -ilc.border)
	-- 	line.item.tex:SetPoint("BOTTOMRIGHT", line.item, "BOTTOMRIGHT", -ilc.border, ilc.border)

	-- 	-- want
	-- 	line.want = line:CreateFontString(nil, "OVERLAY")
	-- 	line.want:SetPoint("LEFT", line.item, "RIGHT", 6, 0)
	-- 	line.want:SetWidth(50)
	-- 	line.want:SetFontObject(ilc:get_font(13))
	...
	-- 	line.gear1.tex:SetPoint("TOPLEFT", line.gear1, "TOPLEFT", ilc.border, -ilc.border)
	-- 	line.gear1.tex:SetPoint("BOTTOMRIGHT", line.gear1, "BOTTOMRIGHT", -ilc.border, ilc.border)
	...
	-- 	line.gear2.tex:SetPoint("TOPLEFT", line.gear2, "TOPLEFT", ilc.border, -ilc.border)
	-- 	line.gear2.tex:SetPoint("BOTTOMRIGHT", line.gear2, "BOTTOMRIGHT", -ilc.border, ilc.border)
	...
	-- 	line.date:SetFontObject(ilc:get_font(13))
	...
	local function create_entry(self)
		...
		entry:SetBackdrop({bgFile = ilc.media.flat})
		entry:SetBackdropColor(0, 0, 0, 0)

		entry.update = entry:CreateTexture("nil", "ARTWORK")
		entry.update:SetTexture(ilc.media.flat)
		...
		entry.name.text:SetFontObject(ilc:get_font(14))
		...
		entry:SetScript("OnClick", function(self)	
			...
			if (vote_table.info_pane:IsShown()) then
				...
				-- set name
				vote_table.info_pane.name:SetText(ilc:capitalize(self.playerName))
				...
				-- -- populate history
				-- local history = ilc:getLootHistory(self.playerName)
		...
		entry.rank:SetFontObject(ilc:get_font(14))
		...
		entry.ilvl:SetFontObject(ilc:get_font(14))
		...
		entry.interest.text:SetFontObject(ilc:get_font(14))
		...
		entry.voteUser:SetScript("OnClick", function()
			vote_table.info_pane:Hide()
			ilc:voteForUser(ilc.localPlayer, entry.itemUID, entry.playerName, true)
			ilc:sendAction("voteForUser", ilc.localPlayer, entry.itemUID, entry.playerName);
		end)
		...
		ilc:skinButton(entry.removeUser,true,"red")
		entry.removeUser:SetScript("OnClick", function()
			vote_table.info_pane:Hide()
			ilc:sendAction("removeUserConsidering", entry.itemUID, entry.playerName);
			ilc:removeUserConsidering(entry.itemUID, entry.playerName)
		end)
		...
		ilc:setBackdrop(entry.gear1, 0, 0, 0, 1)
		...
		entry.gear1.tex:SetPoint("TOPLEFT", entry.gear1, "TOPLEFT", ilc.border, -ilc.border)
		entry.gear1.tex:SetPoint("BOTTOMRIGHT", entry.gear1, "BOTTOMRIGHT", -ilc.border, ilc.border)
		...
		ilc:setBackdrop(entry.gear2, 0, 0, 0, 1)
		...
		entry.gear2.tex:SetPoint("TOPLEFT", entry.gear2, "TOPLEFT", ilc.border, -ilc.border)
		entry.gear2.tex:SetPoint("BOTTOMRIGHT", entry.gear2, "BOTTOMRIGHT", -ilc.border, ilc.border)
		...
		entry.votes.text:SetFontObject(ilc:get_font(14))
		...
		entry.votes:SetScript("OnEnter", function()
			if (tonumber(entry.votes.text:GetText()) > 0) then
				...
				for council, ot in pairs(ilc.loot_council_votes[entry.itemUID]) do
					for v = 1, #ilc.loot_council_votes[entry.itemUID][council] do
						if (ilc.loot_council_votes[entry.itemUID][council][v] == entry.playerName) then
							local name, server = strsplit("-", council)
							GameTooltip:AddLine(ilc:prettyName(name), 1, 1, 1)
						end
					end
				end	
...
ilc.tabs = CreateObjectPool(create_tab, reset_tab)

-- rolls
local i = 0
local function create_roll()
	i = i + 1
	local roll = CreateFrame("frame", nil, ilc.rollFrame, BackdropTemplateMixin and "BackdropTemplate");

	roll:SetPoint("TOPLEFT", ilc.rollFrame, "TOPLEFT", 0, 0)
	roll:SetSize(ilc.rollFrame:GetWidth(), 60);
	roll:EnableMouse(true)
	roll:RegisterForDrag("LeftButton", "RightButton")
	roll:SetScript("OnDragStart", function(self) ilc.rollFrame:StartMoving() end)
	roll:SetScript("OnDragStop", function(self) ilc.rollFrame:StopMovingOrSizing() end)
	roll:SetScript("OnMouseUp", function(self) ilc.rollFrame:StopMovingOrSizing() end)
	ilc:setBackdrop(roll, .1, .1, .1, .9)
	...
	ilc:setBackdrop(roll.item.icon, 0,0,0,.8);
	
	roll.item.icon.wfsock = roll.item.icon:CreateFontString(nil, "ARTWORK")
	roll.item.icon.wfsock:SetFontObject(ilc:get_font(15, "OUTLINE"))
	...
	roll.item.icon.tex:SetPoint("TOPLEFT", roll.item.icon, "TOPLEFT", ilc.border, -ilc.border)
	roll.item.icon.tex:SetPoint("BOTTOMRIGHT", roll.item.icon, "BOTTOMRIGHT", -ilc.border, ilc.border)

	roll.item.item_text = roll.item:CreateFontString(nil, "ARTWORK")
	roll.item.item_text:SetFontObject(ilc:get_font(15, "OUTLINE"))
	...
	roll.item.num_items:SetFontObject(ilc:get_font(12))
	...
	local submit_roll = function(button, wantLevel)
		...
		ilc:sendAction("addUserWant", roll.itemUID, ilc.localPlayer, wantLevel, itemLink1, itemLink2, roll.lootRoll, player_itemlvl, guildRank, notes)
	...
	local function update_notes(roll)
		if (roll.active) then
			...
			if (notes ~= roll.last_note) then
				...
				ilc:sendAction("updateUserNote", roll.itemUID, ilc.localPlayer, notes)
			end
		end
	end
	...
	roll.btns = {}
	for i = 1, 5 do
		local name, colors, enable, req = unpack(ilc.buttons[i])
	...
	ilc:skinButton(roll.buttons.note,false,"blue")
	...
	for i = 1, 10 do
		...
		if (not lastqn) then
			...
			qn:SetPoint("RIGHT", lastqn, "LEFT", ilc.border, 0)
		end
		lastqn = qn
		qn.i = i
		...
		qn:SetScript("OnClick", function(self)
			...
			if (not self.selected) then
				...
				ilc:skinButton(self, true, "blue")
				self.selected = true
			else
				roll.stored_quicknotes[self.i] = nil
				ilc:skinButton(self, true)
				self.selected = false
			end
		end)
	...
	ilc:skinButton(roll.buttons.pass, false, "red")

	roll.buttons.pass:SetScript("OnClick", function()
		ilc:sendAction("removeUserConsidering", roll.itemUID, ilc.localPlayer);
	...
	roll.buttons.notes:SetFontObject(ilc:get_font(14))
	roll.buttons.notes:SetFrameLevel(27)
	roll.buttons.notes:SetBackdrop({bgFile = ilc.media.flat, edgeFile = ilc.media.flat, edgeSize = ilc.border})
	...
	ilc:skinButton(roll.buttons.notes.okay, false, "dark")
	...
	roll.buttons.notes.note_required:SetFontObject(ilc.font)
...
ilc.rolls = CreateObjectPool(create_roll, reset_roll)

--=====================================================
-- Create Main Window
--=====================================================
ilc.window = CreateFrame("frame", "ILC Window", UIParent, BackdropTemplateMixin and "BackdropTemplate")
ilc.window:EnableMouse(true);
ilc.window:SetMovable(true);
ilc.window:SetUserPlaced(true);
ilc.window:SetFrameStrata("DIALOG");
ilc.window:SetFrameLevel(27);
ilc.window:SetSize(600, 400);
ilc.window:SetPoint("CENTER");
ilc.window:Hide();
ilc:setBackdrop(ilc.window, .1, .1, .1, .9);

ilc.window.tabs = CreateFrame("frame", nil, ilc.window);
ilc.window.tabs:SetPoint("TOPLEFT", ilc.window, "TOPLEFT", -40, 0)
ilc.window.tabs:SetPoint("BOTTOMRIGHT", ilc.window, "BOTTOMLEFT", 0, 0)
-- ilc.window:Hide()

ilc.window.header = CreateFrame("frame", nil, ilc.window, BackdropTemplateMixin and "BackdropTemplate");
ilc.window.header:EnableMouse(true);
ilc.window.header:RegisterForDrag("LeftButton","RightButton")
ilc.window.header:SetScript("OnDragStart", function(self) ilc.window:StartMoving() end)
ilc.window.header:SetScript("OnDragStop", function(self)  ilc.window:StopMovingOrSizing() end)
ilc.window.header:SetScript("OnMouseUp", function(self) ilc.window:StopMovingOrSizing() end)
ilc.window.header:SetPoint("TOPLEFT", ilc.window, "TOPLEFT")
ilc.window.header:SetPoint("BOTTOMRIGHT", ilc.window, "TOPRIGHT", 0, -24)
ilc:setBackdrop(ilc.window.header, .1, .1, .1, 1)

ilc.window.header.close = CreateFrame("Button", nil, ilc.window.header, BackdropTemplateMixin and "BackdropTemplate")
ilc.window.header.close:SetPoint("RIGHT", ilc.window.header, "RIGHT", -4, 0)
ilc.window.header.close:SetText("x")
ilc:skinButton(ilc.window.header.close, true, "red")
ilc.window.header.close:SetScript("OnClick", function()
	ilc.window:Hide()
end)

ilc.window.header.text = ilc.window.header:CreateFontString(nil, "OVERLAY")
ilc.window.header.text:SetFontObject(ilc:get_font(15))
ilc.window.header.text:SetText(ilc.colorString)
ilc.window.header.text:SetPoint("CENTER", ilc.window.header, "CENTER")

--==============================================
-- Loot Council Display
--==============================================
ilc.window.loot_council = CreateFrame("frame", nil, ilc.window, BackdropTemplateMixin and "BackdropTemplate")
ilc.window.loot_council:SetPoint("BOTTOMLEFT", ilc.window, "BOTTOMLEFT", 10, 6)
ilc.window.loot_council:SetSize(84, 18)
ilc:setBackdrop(ilc.window.loot_council, .1,.1,.1,.8);
ilc.window.loot_council.text = ilc.window.loot_council:CreateFontString(nil, "OVERLAY")
ilc.window.loot_council.text:SetFontObject(ilc:get_font(14))
ilc.window.loot_council.text:SetPoint("LEFT", ilc.window.loot_council, "LEFT", 4, 0)
ilc.window.loot_council.text:SetText(l["frameLC"])
ilc.window.loot_council.text:SetJustifyH("LEFT")
ilc.window.loot_council.image = ilc.window.loot_council:CreateTexture(nil, "OVERLAY")
ilc.window.loot_council.image:SetTexture("Interface\\FriendsFrame\\InformationIcon")
ilc.window.loot_council.image:SetSize(10, 10)
ilc.window.loot_council.image:SetPoint("RIGHT", ilc.window.loot_council, "RIGHT", -4, 0)
ilc.window.loot_council.image:SetVertexColor(.8,.8,.8)

ilc.window.loot_council.add = CreateFrame("BUTTON", nil, ilc.window.loot_council, BackdropTemplateMixin and "BackdropTemplate")
ilc.window.loot_council.add:SetText(" + ")
ilc:skinButton(ilc.window.loot_council.add,true,"blue")
ilc.window.loot_council.add:SetPoint("LEFT", ilc.window.loot_council, "RIGHT", 2, 0)
ilc.window.loot_council.add:SetWidth(18)
...
ilc.window.loot_council.add:SetScript("OnClick", function()
	StaticPopup_Show("ADD_TO_LC_BOX")
end)

ilc.window:HookScript("OnShow", function()
	if (ilc:IsRaidLeader() or not IsInRaid()) then
		ilc.window.loot_council.add:Show()
	else
		ilc.window.loot_council.add:Hide()
	end
end)

ilc.window.loot_council:SetScript("OnEnter", function()
	ShowUIPanel(GameTooltip)
	GameTooltip:SetOwner(UIParent, "ANCHOR_CURSOR")
	
	for councilName, v in pairs(ilc.loot_council) do
		-- local name, server = strsplit("-", councilName)
		-- if (server == player_realm) then
		-- 	councilName = name
		-- end
		local name, color = ilc:prettyName(councilName)
		GameTooltip:AddLine(name, color.r, color.g, color.b)
	end

	GameTooltip:Show()
end)
ilc.window.loot_council:SetScript("OnLeave", function()
	GameTooltip:Hide()
end)
...
-- Rolls
--==========================================
ilc.rollFrame = CreateFrame('frame', "ILC Roll Window", UIParent)
ilc.rollFrame:SetSize(430, 1)
ilc.rollFrame:SetPoint("CENTER", UIParent, "CENTER", 600, 0)
ilc.rollFrame:EnableMouse(true)
ilc.rollFrame:SetMovable(true);
ilc.rollFrame:SetUserPlaced(true);
ilc.rollFrame:SetFrameStrata("FULLSCREEN");
ilc.rollFrame:SetFrameLevel(10)
ilc.rollFrame:SetClampedToScreen(true);
ilc.rollFrame:RegisterForDrag("LeftButton","RightButton")
ilc.rollFrame:SetScript("OnDragStart", function(self) ilc.rollFrame:StartMoving() end)
ilc.rollFrame:SetScript("OnDragStop", function(self) ilc.rollFrame:StopMovingOrSizing() end)
ilc.rollFrame:SetScript("OnMouseUp", function(self) ilc.rollFrame:StopMovingOrSizing() end)
ilc.rollFrame:Hide()

ilc.rollFrame.title = ilc.rollFrame:CreateFontString(nil, "OVERLAY")
ilc.rollFrame.title:SetFontObject(ilc:get_font(15, "OUTLINE"))
ilc.rollFrame.title:SetText(ilc.colorString)
ilc.rollFrame.title:SetPoint("BOTTOM", ilc.rollFrame, "TOP", 0, 2)